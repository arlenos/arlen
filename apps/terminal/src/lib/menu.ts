/// The terminal app's global menu in the shell's top-left menu bar (the
...
const APP_ID = "dev.arlen.terminal";
...
export function appMenuGroups(t: Translate): MenuGroup[] {
  return [
    {
      label: t("term.menu.session"),
      items: [
        item(t("term.menu.newSession"), "session.new", "Ctrl+T"),
        sep(),
        item(t("term.menu.connect"), "session.connect", "Ctrl+Shift+R"),
      ],
    },
    {
      label: t("term.menu.view"),
      items: [item(t("term.menu.history"), "view.history", "Ctrl+R")],
    },
  ];
}
