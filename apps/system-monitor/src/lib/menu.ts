/// The system-monitor app's global menu in the shell's top-left menu bar (the
...
const APP_ID = "dev.arlen.system-monitor";
...
export function appMenuGroups(t: Translate): MenuGroup[] {
  return [
    {
      label: t("tm.menu.view"),
      items: [
        item(t("tm.menu.refresh"), "view.refresh"),
        sep(),
        item(t("tm.tab.processes"), "view.processes"),
        item(t("tm.tab.performance"), "view.performance"),
      ],
    },
  ];
}
