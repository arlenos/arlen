//! Arlen greeter: the pure login logic behind the Tauri host
//! (`greeter-onboarding-plan.md` §2). This crate holds everything the login
//! screen decides WITHOUT Tauri/webkit - Wayland-session discovery, `/etc/passwd`
//! login-profile enumeration + AccountsService avatars, the greetd auth
//! conversation, and the power-action map - so it is unit-tested in CI (the
...
pub const MAX_AUTH_STEPS: usize = 8;
...
pub fn wayland_session_dirs() -> Vec<PathBuf> {
...
pub fn session_name_from_desktop(contents: &str) -> Option<String> {
...
pub fn discover_sessions(dirs: &[PathBuf]) -> Vec<Session> {
...
pub fn fallback_session() -> Session {
...
/// PATH.
pub fn fallback_command() -> Vec<String> {
...
pub fn session_exec_from_desktop(contents: &str) -> Option<Vec<String>> {
    let mut in_entry = false;
...
pub fn session_command(dirs: &[PathBuf], id: &str) -> Option<Vec<String>> {
...
pub fn parse_login_accounts(passwd: &str, min_uid: u32, max_uid: u32) -> Vec<Profile> {
...
pub const ACCOUNTS_ICONS_DIR: &str = "/var/lib/AccountsService/icons";
...
pub fn resolve_avatar(icons_dir: &Path, username: &str) -> Option<String> {
    use base64::Engine;
    use std::io::Read;
...
pub fn power_verb(action: &str) -> Option<&'static str> {
    match action {
