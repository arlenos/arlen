/// The viewers app's global menu in the shell's top-left menu bar (the
...
const APP_ID = "dev.arlen.viewers";
  ...
  return [
    {
      label: t("v.menu.file"),
      items: [
        item(t("v.print"), "file.print"),
        sep(),
        item(t("v.menu.trash"), "file.trash", "Del"),
        item(t("v.menu.undoDelete"), "file.undo", "Ctrl+Z"),
      ],
    },
    {
      label: t("v.menu.go"),
      items: [
        item(t("v.nextFile"), "go.next"),
        item(t("v.prevFile"), "go.previous"),
      ],
    },
  ];
}
