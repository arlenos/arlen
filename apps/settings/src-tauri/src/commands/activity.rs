...
//!
//! The audit ledger (`lunaris-auditd`) is the durable record of every
//! AI action: natural-language queries, MCP tool calls, graph access,
//! policy violations, confirmations, and permission/network events.
//! This command reads the **Structural tier** (content-free metadata,
//! never Forensic) over the daemon's read socket and exposes the most
//! recent entries to the Settings AI Activity view.
//!
//! It is read-only and advisory: a missing or unreachable daemon
//! yields an empty activity list flagged `available = false`, so the
//! page still renders instead of erroring.

use audit_proto::{read_socket_path, AuditKind, ReadClient, StructuralView};
use serde::Serialize;

/// The largest activity page the view may request. Matches the
/// daemon's own page ceiling so a single tail read returns everything
/// asked for.
const MAX_ACTIVITY_LIMIT: u64 = 200;

/// One audit entry as the Activity view consumes it. Mirrors the
/// Structural tier; camelCase for the frontend.
...
/// The Activity view payload: the most recent entries (newest first)
/// plus daemon liveness and tamper status, so the page renders its
...
/// ledger `head`), then, if the ledger is larger than `limit`, seeks
/// to the tail (`from = head - limit`) with one more read. Both reads
/// are advisory: any failure degrades to an empty, `available = false`
/// page rather than failing the command.
#[tauri::command]
pub async fn ai_activity_recent(limit: u64) -> Result<ActivityPage, String> {
    ...
    // The first read returns the OLDEST `limit` entries. When the
    // ledger holds more, seek to the tail for the newest ones. `head`
    // only grows (the ledger is append-only), so the computed `from`
    ...
    // The API returns ascending by index; the view wants newest first.
    entries.reverse();
...
/// A stable, frontend-friendly label for each audit kind. Exhaustive
/// on purpose: a new `AuditKind` variant must be given a label here
/// rather than silently rendering as a fallback.
