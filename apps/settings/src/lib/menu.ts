/// The settings app's global menu in the shell's top-left menu bar (the
/// knowledge pattern): the whole panel tree as a Go menu - the same SECTIONS
/// the sidebar renders, one source - and the settings search. Registered per
/// translator value so a locale switch re-registers; under vite both calls
/// fail silently and the menu is simply absent. Live it needs
/// `arlen-shell:allow-menu-register` / `-unregister` in this app's
/// capabilities file, and the plugin's menu-action return channel - the same
/// routed seams as the pdf reader's.
import { invoke } from "@tauri-apps/api/core";
import { listen } from "@tauri-apps/api/event";
import { writable } from "svelte/store";
import type { Translate } from "@arlen/ui-kit/i18n";
import { t } from "$lib/i18n/messages";
import { PANELS, SECTIONS } from "$lib/stores/navigation";

const APP_ID = "dev.arlen.settings";
...
export function appMenuGroups(t: Translate): MenuGroup[] {
  return [
    {
      label: t("s.menu.view"),
      items: [item(t("s.search.placeholder"), "view.search")],
    },
    {
      label: t("s.menu.go"),
