...
/// it already holds, so a region never has to be asked for twice and the picture
/// a person crops is the one they saw. Region capture is REACHABLE that way; what
/// this exists for is the other flow, where a rectangle is dragged BEFORE
/// anything is captured. That surface has not been built.
#[tauri::command]
fn capture_region(
    output_index: usize,
    x: u32,
