/// The store app's global menu in the shell's top-left menu bar (the
...
export function appMenuGroups(t: Translate): MenuGroup[] {
  return [
    {
      label: t("st.menu.store"),
      items: [
        item(t("st.menu.refresh"), "store.refresh"),
        sep(),
        item(t("st.menu.checkUpdates"), "store.check"),
        item(t("st.upd.updateAll"), "store.update_all"),
      ],
    },
    {
      label: t("st.menu.go"),
      items: [
        item(t("st.rail.browse"), "go.browse"),
        item(t("st.rail.installed"), "go.installed"),
        item(t("st.rail.updates"), "go.updates"),
      ],
    },
  ];
}
