/// The Knowledge app message catalog, authored in MessageFormat 2.0. English is the
/// source of truth; German proves the reactive locale switch. Follows the meetings /
/// system-monitor template. Key prefix: `k.`.
import { createTranslator, type Catalogs } from "@arlen/ui-kit/i18n";

/// Re-exported so the app's locale control (and tests) drive the same shared store.
export { locale, dir } from "@arlen/ui-kit/i18n";

const messages: Catalogs = {
  en: {
    "k.app.title": "Knowledge",
    "k.nav.aria": "Views",
    "k.menu.timeline": "Timeline",
    // The quiet leading verb of a timeline row, per event kind.
    "k.pv.verb.openedIn": "opened in",
    "k.tl.verb.opened": "opened",
    "k.tl.verb.edited": "edited",
    "k.tl.verb.ran": "ran",
    "k.tl.verb.focused": "focused",
    "k.tl.verb.tagged": "tagged",
    "k.tl.verb.imported": "imported",
    "k.menu.export": "Export recorded activity…",
    "k.menu.deleteToday": "Delete today's activity…",
    "k.menu.deleteAll": "Delete everything recorded…",
    "k.title": "Knowledge",
    "k.sample": "Example data, not this machine's graph.",
    // Distinct from the sample line above: one says the graph is not there yet,
    // the other says this search did not run. A failed read used to show the
    // first, with invented rows under it.
    "k.se.unavailable": "That search did not run.",
    "k.browse.unavailable": "Your graph cannot be read.",
    // The kit's FileBrowser defaults to English and to the word "folder".
    // Neither is right here: these places are the graph answering, and the
    // default reached every locale because nothing overrode it.
    "k.fb.errorTitle": "Cannot show this",
    "k.fb.hintUnknown": "Something went wrong reading your graph.",
    "k.fb.browserLabel": "Graph browser",
    // Reachable: the kit picks a hint by matching the error text, and the
    // knowledge daemon answers an out-of-scope read with a permission denial.
    "k.fb.hintPermission": "Your access to the graph does not cover this.",
    "k.fb.hintNotConnected": "The knowledge graph is not reachable right now.",
    "k.fb.hintNoSuchDir": "This is no longer in your graph.",
    "k.library.unavailable": "Your library cannot be read.",
    "k.capsules.unavailable": "Your capsules cannot be read. This is not a list of none.",
    "k.settingsOpenFailed": "Settings could not be opened.",
    "k.settingsNotInstalled": "Settings is not installed on this machine, so the capability browser cannot be opened from here.",
    "k.capsules.mintFailed": "The capsule was not minted, so nothing has been shared.",
    "k.capsules.revokeFailed": "The capsule was not revoked and can still be read.",
    "k.section.explore": "Explore",
    "k.section.authority": "Authority",
    "k.place.timeline": "Timeline",
    "k.place.projects": "Projects",
    "k.place.searches": "Searches",
    "k.place.library": "Library",
    "k.place.capsules": "Capsules",
    "k.place.capabilities": "Capabilities",
    "k.caps.opens": "Opens in Settings",
    "k.empty.timeline": "Nothing recorded here.",
    "k.tl.noService": "The knowledge service is not running, so nothing is being recorded.",
    "k.tl.unavailable": "Your timeline cannot be read.",
    "k.empty.projects": "No projects detected.",
    "k.projects.unavailable": "Your projects cannot be read.",
    // A column INSIDE a project, which is a different sentence from the one above
    // it: the root's "No projects detected yet." was being shown for a project's
    // members, where it is about the wrong thing and false besides - the projects
    // are listed one column over.
    "k.projects.noMembers": "Nothing is recorded as part of this project.",
    "k.projects.membersUnavailable": "What belongs to a project cannot be read.",
    "k.empty.searches": "No saved searches.",
    "k.searches.unavailable": "Your saved searches cannot be read.",
    "k.empty.library": "No sources bridged in. Bridges come from the Store and bring their source with them.",
    "k.detail.title": "Details",
    "k.detail.none": "Select something to see its details.",
    "k.detail.when": "When",
    "k.detail.source": "Source",
    "k.detail.project": "Project",
    "k.detail.showMore": "Show {$n} more",
    "k.close": "Close",
    "k.tl.pause": "Pause",
    "k.tl.resume": "Resume",
    "k.tl.pausedLine": "Recording is paused. Nothing is added until you resume.",
    "k.tl.pauseUnavailable": "Recording could not be paused, so it is still running.",
    "k.tl.whats": "What is recorded",
    "k.tl.statement": "Typed events only: files opened, edits, commands run, window focus and agent actions. Never screenshots and never file contents.",
    "k.tl.exportFail": "The timeline could not be written to a file.",
    "k.tl.deleteFail": "Nothing was deleted. The recorded range is still there.",
    "k.tl.exportedTo": "Written to {$path}",
    "k.tl.menuHint": "Export and delete live in the app menu.",
    "k.tl.deleteTitle": "Delete recorded activity?",
    "k.tl.deleteMsg": "Removes {$range} from the timeline. This cannot be undone.",
    "k.tl.deleteMsgCounted":
      ".input {$n :number}\n.match $n\none {{Removes 1 record of {$range} from the timeline. This cannot be undone.}}\n*   {{Removes {$n} records of {$range} from the timeline. This cannot be undone.}}",
    "k.tl.deleteConfirm": "Delete",
    "k.tl.rangeToday": "today's activity",
    "k.tl.rangeAll": "everything recorded",
    "k.tl.session": "Session",
    "k.tl.sessionMeta": ".input {$n :number}\n.match $n\none {{1 event}}\n*   {{{$n} events}}",
    "k.tl.span": "{$from} to {$to}",
    "k.tl.scrubAria": "Jump to a day",
    "k.li.noService": "The knowledge service is not running.",
    "k.li.origin": "{$source}, {$n}",
    "k.se.placeholder": "Search your graph",
    "k.se.aria": "Search across everything recorded",
    "k.se.facet.type": "Type",
    "k.se.facet.project": "Project",
    "k.se.facet.time": "Time",
    "k.se.clearFacet": "Clear this filter",
    "k.se.noProjects": "No projects detected.",
    "k.se.projectsUnavailable": "Projects cannot be read.",
    "k.se.today": "Today",
    "k.se.week": "This week",
    "k.se.month": "This month",
    "k.se.type.file": "File",
    "k.se.type.project": "Project",
    "k.se.type.paper": "Paper",
    "k.se.type.mail": "Mail",
    "k.se.type.note": "Note",
    "k.se.type.session": "Session",
    // NOT "by name". The rows are exactly the ids the graph's ranker returned
    // (search.rs:109 pins `f.id IN [...]`), and that ranker fuses keyword hits over
    // an item's synthesized text - path, basename, parent, project name - with the
    // items LINKED to those hits. Searching README on 16 August returned README.md,
    // the project it sits in and a sibling Cargo.lock: two of the three carry no
    // "README" anywhere, so the old caption was refuted by its own result list.
    "k.se.byName": "Matching by name, path and what they link to",
    "k.se.save": "Save this search",
    "k.se.saveFailed": "That search was not saved.",
    "k.se.saveConfirm": "Save",
    "k.se.saveNamePlaceholder": "Name this search",
    "k.se.saved": "Saved searches",
    "k.se.none": "Nothing matches. Try fewer filters or another name.",
    "k.pr.asof": "As of",
    "k.pr.asofAria": "Show the projects as they were at an earlier moment",
    "k.pr.asofActive": "As of: {$day}",
    "k.pr.asofClear": "Back to now",
    "k.detail.provenance": "Provenance",
    "k.detail.provenanceUnavailable": "Where this came from could not be read.",
    "k.detail.members": "Members",
    "k.detail.membersVal": ".input {$n :number}\n.match $n\none {{1 file}}\n*   {{{$n} files}}",
    "k.detail.detected": "Detected from your activity, {$when}",
    "k.detail.recent": "Recent activity",
  },
  de: {
    "k.app.title": "Wissen",
    "k.nav.aria": "Ansichten",

    "k.menu.timeline": "Zeitleiste",
    "k.pv.verb.openedIn": "geöffnet in",
    "k.tl.verb.opened": "geöffnet",
    "k.tl.verb.edited": "bearbeitet",
    "k.tl.verb.ran": "ausgeführt",
    "k.tl.verb.focused": "im Fokus",
    "k.tl.verb.tagged": "verschlagwortet",
    "k.tl.verb.imported": "importiert",
    "k.menu.export": "Aufgezeichnete Aktivität exportieren…",
    "k.menu.deleteToday": "Heutige Aktivität löschen…",
    "k.menu.deleteAll": "Alles Aufgezeichnete löschen…",
    "k.title": "Wissen",
    "k.sample": "Beispieldaten, nicht der Graph dieses Rechners.",
    "k.se.unavailable": "Diese Suche ist nicht gelaufen.",
    "k.browse.unavailable": "Dein Graph kann nicht gelesen werden.",
    "k.fb.errorTitle": "Kann nicht angezeigt werden",
    "k.fb.hintUnknown": "Beim Lesen deines Graphen ist etwas schiefgegangen.",
    "k.fb.browserLabel": "Graph-Browser",
    "k.fb.hintPermission": "Dein Zugriff auf den Graphen deckt das nicht ab.",
    "k.fb.hintNotConnected": "Der Knowledge Graph ist gerade nicht erreichbar.",
    "k.fb.hintNoSuchDir": "Das ist nicht mehr in deinem Graphen.",
    "k.library.unavailable": "Deine Bibliothek kann nicht gelesen werden.",
    "k.capsules.unavailable": "Deine Kapseln k\u00f6nnen nicht gelesen werden. Das hei\u00dft nicht, dass es keine gibt.",
    "k.settingsOpenFailed": "Die Einstellungen konnten nicht geöffnet werden.",
    "k.settingsNotInstalled": "Die Einstellungen sind auf diesem Rechner nicht installiert, deshalb lässt sich die Berechtigungsübersicht von hier nicht öffnen.",
    "k.capsules.mintFailed": "Die Kapsel wurde nicht erstellt, es wurde also nichts geteilt.",
    "k.capsules.revokeFailed": "Die Kapsel wurde nicht widerrufen und kann weiterhin gelesen werden.",
    "k.section.explore": "Erkunden",
    "k.section.authority": "Zugriff",
    "k.place.timeline": "Verlauf",
    "k.place.projects": "Projekte",
    "k.place.searches": "Suchen",
    "k.place.library": "Bibliothek",
    "k.place.capsules": "Kapseln",
    "k.place.capabilities": "Berechtigungen",
    "k.caps.opens": "Öffnet in Einstellungen",
    "k.empty.timeline": "Hier ist nichts aufgezeichnet.",
    "k.tl.noService": "Der Wissensdienst läuft nicht, es wird also nichts aufgezeichnet.",
    "k.tl.unavailable": "Deine Chronik kann nicht gelesen werden.",
    "k.empty.projects": "Keine Projekte erkannt.",
    "k.projects.unavailable": "Deine Projekte k\u00f6nnen nicht gelesen werden.",
    "k.projects.noMembers": "Zu diesem Projekt ist nichts erfasst.",
    "k.projects.membersUnavailable": "Was zu einem Projekt gehört, kann nicht gelesen werden.",
    "k.empty.searches": "Keine gespeicherten Suchen.",
    "k.searches.unavailable": "Deine gespeicherten Suchen können nicht gelesen werden.",
    "k.empty.library": "Keine Quellen eingebunden. Brücken kommen aus dem Store und bringen ihre Quelle mit.",
    "k.detail.title": "Details",
    "k.detail.none": "Wähle etwas, um die Details zu sehen.",
    "k.detail.when": "Wann",
    "k.detail.source": "Quelle",
    "k.detail.project": "Projekt",
    "k.detail.showMore": "{$n} weitere zeigen",
    "k.close": "Schließen",
    "k.tl.pause": "Pausieren",
    "k.tl.resume": "Fortsetzen",
    "k.tl.pausedLine": "Aufzeichnung pausiert. Bis zum Fortsetzen kommt nichts dazu.",
    "k.tl.pauseUnavailable": "Die Aufzeichnung konnte nicht pausiert werden und läuft weiter.",
    "k.tl.whats": "Was aufgezeichnet wird",
    "k.tl.statement": "Nur typisierte Ereignisse: geöffnete Dateien, Bearbeitungen, ausgeführte Befehle, Fensterfokus und Agent-Aktionen. Nie Screenshots und nie Dateiinhalte.",
    "k.tl.exportFail": "Die Chronik konnte nicht in eine Datei geschrieben werden.",
    "k.tl.deleteFail": "Es wurde nichts gelöscht. Der aufgezeichnete Zeitraum ist weiterhin vorhanden.",
    "k.tl.exportedTo": "Geschrieben nach {$path}",
    "k.tl.menuHint": "Exportieren und Löschen liegen im App-Menü.",
    "k.tl.deleteTitle": "Aufgezeichnete Aktivität löschen?",
    "k.tl.deleteMsg": "Entfernt {$range} aus dem Verlauf. Das lässt sich nicht rückgängig machen.",
    "k.tl.deleteMsgCounted":
      ".input {$n :number}\n.match $n\none {{Entfernt 1 Eintrag aus {$range} im Verlauf. Das lässt sich nicht rückgängig machen.}}\n*   {{Entfernt {$n} Einträge aus {$range} im Verlauf. Das lässt sich nicht rückgängig machen.}}",
    "k.tl.deleteConfirm": "Löschen",
    "k.tl.rangeToday": "die heutige Aktivität",
    "k.tl.rangeAll": "alles Aufgezeichnete",
    "k.tl.session": "Sitzung",
    "k.tl.sessionMeta": ".input {$n :number}\n.match $n\none {{1 Ereignis}}\n*   {{{$n} Ereignisse}}",
    "k.tl.span": "{$from} bis {$to}",
    "k.tl.scrubAria": "Zu einem Tag springen",
    "k.li.noService": "Der Wissensdienst läuft nicht.",
    "k.li.origin": "{$source}, {$n}",
    "k.se.placeholder": "Deinen Graphen durchsuchen",
    "k.se.aria": "Über alles Aufgezeichnete suchen",
    "k.se.facet.type": "Typ",
    "k.se.facet.project": "Projekt",
    "k.se.noProjects": "Keine Projekte erkannt.",
    "k.se.projectsUnavailable": "Projekte können nicht gelesen werden.",
    "k.se.facet.time": "Zeit",
    "k.se.clearFacet": "Diesen Filter entfernen",
    "k.se.today": "Heute",
    "k.se.week": "Diese Woche",
    "k.se.month": "Diesen Monat",
    "k.se.type.file": "Datei",
    "k.se.type.project": "Projekt",
    "k.se.type.paper": "Paper",
    "k.se.type.mail": "Mail",
    "k.se.type.note": "Notiz",
    "k.se.type.session": "Sitzung",
    "k.se.byName": "Treffer nach Name, Pfad und Verknüpftem",
    "k.se.save": "Diese Suche speichern",
    "k.se.saveFailed": "Diese Suche wurde nicht gespeichert.",
    "k.se.saveConfirm": "Speichern",
    "k.se.saveNamePlaceholder": "Suche benennen",
    "k.se.saved": "Gespeicherte Suchen",
    "k.se.none": "Nichts passt. Versuch weniger Filter oder einen anderen Namen.",
    "k.pr.asof": "Stand vom",
    "k.pr.asofAria": "Die Projekte zeigen, wie sie zu einem früheren Zeitpunkt waren",
    "k.pr.asofActive": "Stand vom {$day}",
    "k.pr.asofClear": "Zurück zu jetzt",
    "k.detail.provenance": "Herkunft",
    "k.detail.provenanceUnavailable": "Woher das stammt, ließ sich nicht lesen.",
    "k.detail.members": "Mitglieder",
    "k.detail.membersVal": ".input {$n :number}\n.match $n\none {{1 Datei}}\n*   {{{$n} Dateien}}",
    "k.detail.detected": "Aus deiner Aktivität erkannt, {$when}",
    "k.detail.recent": "Letzte Aktivität",
  },
};

/// The reactive translator bound to the shared locale store.
export const t = createTranslator(messages);
