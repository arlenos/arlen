...
<div class="tl">
  ...
  <div class="tl-scroll" bind:this={scroller} onscroll={onScroll}>
    {#if empty}
      <!-- One empty spine, two different things being said in it. A timeline
           with nothing in it is a FACT and stays quiet text; a timeline that
           could not be read is a REFUSAL and is drawn as one, the way Mail, the
           Calendar and the Task manager draw theirs. The sentences already
           differed; only the presentation said "nothing here" for both, and the
           presentation is what somebody skimming reads. -->
      {#if $timelineNoService || $timelineUnavailable}
        <div class="tl-empty">
          <Notice
            tone="error"
            text={$timelineNoService ? $t("k.tl.noService") : $t("k.tl.unavailable")}
          />
        </div>
      {:else}
        <p class="tl-empty">{$t("k.empty.timeline")}</p>
      {/if}
    {:else if $days}
