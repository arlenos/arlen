//! The sandboxed image-thumbnail worker.
//!
//! Locks itself down with [`arlen_ai_sandbox::apply_sandbox`] (no new
//! privileges, no filesystem, no network), then reads untrusted image bytes
//! from stdin, decodes and downscales them to a thumbnail, and writes the PNG
//! to stdout. Image decoders are a memory-unsafe attack surface, so the decode
//! only ever happens here, behind the lockdown; the parent
//! ([`arlen_ai_sandbox::thumbnail`]) feeds it the untrusted image and reads back
//! only the re-encoded thumbnail.
//!
//! A non-zero exit means no thumbnail was produced; the parent treats that as
//! fail-closed. This binary is built only with the `thumbnail` feature.
...
fn main() {
    ...
    // otherwise survive into the worker and remain usable by exploited decoder
    // code. (Mirrors `arlen-doc-sandbox`; kept per-worker as the local lockdown
    // step.)
    close_inherited_fds();

    // Lock down before touching any untrusted input. If the sandbox cannot be
    // installed we refuse to decode rather than run exposed.
    ...
    match arlen_ai_sandbox::generate_thumbnail(&input, arlen_ai_sandbox::THUMBNAIL_MAX_DIM) {
        Ok(thumb) => {
...
fn main() {
    eprintln!("arlen-thumbnail-sandbox requires Linux (Landlock + seccomp)");
    std::process::exit(2);
}
