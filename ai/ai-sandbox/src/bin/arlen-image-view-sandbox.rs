//! The sandboxed full-resolution image-view worker (`apps/viewers`).
//!
...
//! from stdin, decodes them at full resolution (downscaling only past
//! [`arlen_ai_sandbox::VIEWER_MAX_DIM`]), and writes the sanitised PNG to
//! stdout. Image decoders are a memory-unsafe attack surface, so the decode only
//! ever happens here, behind the lockdown; the parent
//! ([`arlen_ai_sandbox::view_image`]) feeds it the untrusted image and reads
//! back only the re-encoded PNG.
//!
//! A non-zero exit means no image was produced; the parent treats that as
//! fail-closed. This binary is built only with the `thumbnail` feature (it
//! shares the image decoders).
...
fn main() {
    ...
    match arlen_ai_sandbox::decode_view_image(&input) {
        ...
        Err(e) => {
            eprintln!("image view decode failed: {e}");
            std::process::exit(5);
        }
    }
}
...
fn main() {
    eprintln!("arlen-image-view-sandbox requires Linux (Landlock + seccomp)");
    std::process::exit(2);
}
