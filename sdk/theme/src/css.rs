...
//! `BTreeMap<String, String>` of CSS custom properties (without the `--`
//! prefix), plus helpers to produce the injectable `:root` string. This is
//! the single shared generator: the desktop shell (the theme authority) and
//! every other Arlen app's theme consumer emit the same variables, so a
//! theme switch reskins the whole desktop identically (GAP-20).
//!
//! Radius emission applies the user's intensity multiplier via
//! `ArlenTheme::effective_*()` so the variables observed by the webview are
//! already-scaled. `--radius-full` is categorical and bypasses intensity.
...
/// Convert a resolved theme into the flat CSS-var set. `font_scale` is the
/// user's optional override (`None` = the 1.0 default); it is carried on the
/// result and applied by [`to_css_string`].
pub fn to_css_variables(theme: &ArlenTheme, font_scale: Option<f32>) -> CssVariables {
...
/// Generate an injectable `:root { ... }` CSS string. The font-size override
/// only emits when the user picked a non-1.0 scale.
...
/// Format an `[r, g, b, a]` (each `0.0..=1.0`) as a CSS color string. Uses
/// `#rrggbb` for opaque, `rgba(...)` for translucent.
...
mod tests {
    use super::*;
    use crate::{DARK_TOML, LIGHT_TOML};
    ...
    fn generates_all_expected_keys() {
        let css = to_css_variables(&dark(), None);
    ...
    fn radius_intensity_applied_in_emission() {
        ...
        let css = to_css_variables(&theme, None);
        // chip(4) * 2.0 = 8
    ...
    fn font_scale_injected() {
        let theme = ArlenTheme::from_bundled(LIGHT_TOML).expect("bundled light resolves");
        let css = to_css_variables(&theme, Some(1.25));
        let output = to_css_string(&css);
