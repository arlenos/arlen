/**
 * @arlen/tauri-plugin-menu
 *
 * TypeScript bindings for the Arlen titlebar protocol.
...
export async function setTitle(title: string): Promise<void> {
  await invoke("plugin:arlen-menu|set_title", { title });
}
...
export async function setBreadcrumb(
  ...
  await invoke("plugin:arlen-menu|set_breadcrumb", {
...
export async function setCenterContent(
  ...
  await invoke("plugin:arlen-menu|set_center_content", {
...
export async function addTab(tab: Tab): Promise<void> {
  await invoke("plugin:arlen-menu|add_tab", {
    tab: {
      id: tab.id,
      title: tab.title,
      icon: tab.icon ?? null,
...
export async function removeTab(id: string): Promise<void> {
  await invoke("plugin:arlen-menu|remove_tab", { id });
}
...
export async function updateTab(
  ...
  await invoke("plugin:arlen-menu|update_tab", {
    id,
    title,
...
export async function addButton(button: TitlebarButton): Promise<void> {
  await invoke("plugin:arlen-menu|add_button", {
    id: button.id,
    icon: button.icon,
...
export async function removeButton(id: string): Promise<void> {
  await invoke("plugin:arlen-menu|remove_button", { id });
}
...
export async function setButtonEnabled(
  ...
  await invoke("plugin:arlen-menu|set_button_enabled", { id, enabled });
}
...
export async function setSearchMode(enabled: boolean): Promise<void> {
  await invoke("plugin:arlen-menu|set_search_mode", { enabled });
}
...
export function onModeChanged(
  ...
  return listen<string>("arlen-titlebar://mode-changed", ({ payload }) => {
...
export function onTabActivated(
  ...
  return listen<string>("arlen-titlebar://tab-activated", ({ payload }) => {
    callback(payload);
  });
}
...
export function onTabClosed(
  ...
  return listen<string>("arlen-titlebar://tab-closed", ({ payload }) => {
    callback(payload);
  });
}
...
export function onTabReordered(
  ...
  return listen<string>("arlen-titlebar://tab-reordered", ({ payload }) => {
    try {
...
export function onButtonClicked(
  ...
  return listen<string>("arlen-titlebar://button-clicked", ({ payload }) => {
    callback(payload);
  });
}
...
export function onSearchChanged(
  ...
  return listen<string>("arlen-titlebar://search-changed", ({ payload }) => {
    callback(payload);
  });
}
...
export function onKeyboardAction(
  ...
  return listen<string>(
    "arlen-titlebar://keyboard-action",
    ({ payload }) => {
      callback(payload);
    }
  );
 ...
 *
 * Subscribes to `arlen-titlebar://mode-changed` on first consumer and
 * tears down when the last unsubscribes. Initial value is `"floating"`
 * so non-Arlen compositors (GNOME, KDE, etc.) get sensible defaults.
 *
 * Svelte usage:
