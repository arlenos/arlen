...
async fn emit_open(path: &str) -> Result<(), String> {
    ...
    .encode_to_vec();
    // `system:dogfood`, not a session reference. This binary stands in for a
    // sensor and is not a user: its events are generated by the verify image
    // about itself, so naming the producer is the truthful origin. It also has
    // no session id to read - it starts from default.target, before anything
    // mints one, and moving it later so it could inherit one stopped it running
    // at all, which is how this was settled.
    UnixEventEmitter::for_system_named("dogfood", socket)
