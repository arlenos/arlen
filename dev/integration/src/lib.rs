...
mod crate_reachability {
    ...
    const KNOWN_UNCONSUMED: &[(&str, &str)] = &[
        ...
        ("apps/mail/core",
         "The mail client's stated rules for what the standards left ambiguous, built 19 August: the duplicate Content-Type / Content-Transfer-Encoding rule from the LangSec differential-fuzzing finding, the (identity, UIDVALIDITY, UID) sync key with the pending-action drop RFC 4549 requires, the multipart/alternative divergence notice, and the inventory of every channel a message can phone home through. Nothing consumes it because there is no client yet - no IMAP, no store, no window - and the plan's own scope section says section 6 produced no surviving claims and needs a second pass, so building a surface now would be convention dressed as research. The rules are worth having first because they are the part that was measured."),
        ("ai/ai-explanation",
