...
mod crate_reachability {
    ...
    const KNOWN_UNCONSUMED: &[(&str, &str)] = &[
        ...
        ("apps/pdf/core",
         "The PDF reader's document model, built 19 August: page count, the author's outline for a table of contents, and page text for in-document search. Nothing consumes it because the reader's Tauri host is the next slice, and the split is deliberate - rendering a page needs PDFium, which is a system library and therefore a new line in `ci-system-packages.txt`, the file the apt cache key hashes. That makes every job cold, which is a cost to pay on purpose for the rasteriser rather than as a side effect of starting the reader."),
        ("ai/ai-explanation",
