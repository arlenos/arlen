...
KNOWN = {
    "arlen-graph": (
        ...
        "    What remains: a caller the broker has never heard of - anything "
        "outside the stamped-unit table, such as a verify probe - still falls to "
        "/proc and is refused there. Carried because that is the correct answer "
        "for an unstamped caller rather than a defect, and closing it means "
        "deciding what may be stamped, not editing this daemon."
    ),
    "arlen-event-bus": (
        "SAME SHAPE, SAME FIX, arrived 15 Aug with the bus moving per-user. It "
        "asks the broker first and keeps /proc beneath, exactly as the graph "
        "daemon above does.\n"
        "    Worth stating that this one was self-inflicted and predicted: moving "
        "the bus out from under the system manager is what took away the root "
        "exemption its /proc reads had been relying on, and this check said so in "
        "the same run that made the change. The identity is what the bus scopes "
        "publish and subscribe by, so an unresolvable peer means a producer held "
        "to nothing and a consumer filtered to nothing."
    ),
