...
KNOWN = {
    "arlen-ai-undo-signer": (
        ...
        "    Two ways out, both named by the planner, neither taken here. Drop the "
        "mount hardening: one line, works today, and argues against the custody "
        "reason written above - not a call to make by editing a unit at the end of "
        "a night. Or pass the daemon a socket already CONNECTED to the broker at "
        "spawn, which removes the authentication step instead of weakening it, and "
        "needs the session to start this daemon rather than the user manager.\n"
        "    Do not delete this entry when apps launch stamped. Delete it when one "
        "of those two lands."
    ),
    "arlen-notifyd": (
