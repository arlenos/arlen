...
CARRIED: dict[str, tuple[int, str]] = {
    ...
    "settings": (11, "the 8 September scan, re-measured per-app, less `theme_get`, `keybindings_get_defaults` and `keybindings_get_all_conflicts` (all deleted - each a second name for something a live command already returns) `settings_app_audit` (an app's page shows what it did, beside what it may reach) and `theme_contrast_report` (the accessibility page audits the active appearance). The three `extensions_*` commands are the backend of the management surface the shell-extension strand named, so a strand rather than a loose end; three theme readers; and the ai/keybinding readers"),
