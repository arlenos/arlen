...
CARRIED: dict[str, tuple[int, str]] = {
    "desktop-shell": (24, "the 8 September scan, re-measured per-app. Three clusters: the six `qs_layout_*` writers (a SECOND writer for a file Settings already edits through `config_set` - which side owns the layout's invariants is a decision, not a deletion), the global-menu registry (`register_menu`/`set_menu_state`/`unregister_menu` - the shell's cross-app menu path, which no app uses because each declares its menu to its own backend), and features with no UI at all"),
    ...
    "settings": (16, "the 8 September scan, re-measured per-app. The three `extensions_*` commands are the backend of the management surface the shell-extension strand named, so a strand rather than a loose end; four theme readers; and the ai/keybinding readers"),
