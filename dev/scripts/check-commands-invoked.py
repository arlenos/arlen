...
CARRIED: dict[str, tuple[int, str]] = {
    ...
    "settings": (15, "the 8 September scan, re-measured per-app, less `theme_get` (deleted - a second name for a read the generic config path already does). The three `extensions_*` commands are the backend of the management surface the shell-extension strand named, so a strand rather than a loose end; four theme readers; and the ai/keybinding readers"),
