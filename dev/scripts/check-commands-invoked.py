...
CARRIED: dict[str, tuple[int, str]] = {
    ...
    "settings": (12, "the 8 September scan, re-measured per-app, less `theme_get`, `keybindings_get_defaults` and `keybindings_get_all_conflicts` (all deleted - each a second name for something a live command already returns) and `settings_app_audit` (an app's page shows what it did, beside what it may reach). The three `extensions_*` commands are the backend of the management surface the shell-extension strand named, so a strand rather than a loose end; four theme readers; and the ai/keybinding readers"),
