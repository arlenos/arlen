...
CARRIED: dict[str, tuple[int, str]] = {
    ...
    "files": (1, "the 8 September scan, less `files_unmount` which answered for itself. What is left is `network_places`: the sidebar has no Network section, though the command already separates \"no accounts\" from \"the subsystem is not there\""),
