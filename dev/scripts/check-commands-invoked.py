...
CARRIED: dict[str, tuple[int, str]] = {
    "desktop-shell": (14, "the 8 September scan, re-measured per-app, less `connect_hidden_network` (the popover can join a hidden network now). What is left splits three ways: the six `qs_layout_*` writers (a second writer for a file Settings already edits, so which side owns the layout invariants is a decision not a deletion), the global-menu registry - `register_menu`/`set_menu_state`/`unregister_menu` - which no app uses because each declares its menu to its own backend, and features with no surface at all"),
