...
say "with nothing behind it, it says it cannot read your meetings" \
  "$(printf '%s' "$out" | grep -q "Cannot read your meetings" && echo 1 || echo 0)" "$out"

# THE case. The generic sentence passes the line above whether or not the cause
# survived the catch, so this asks for the cause itself: the colon and something
# after it. This is the assertion that fails if `meetingsFailure` is ever dropped
# again on the way from the store to the surface.
say "and it names why, rather than shrugging" \
  "$(printf '%s' "$out" | grep -qE "Cannot read your meetings[^\"]*: [^ \"][^\"]*" && echo 1 || echo 0)" "$out"
