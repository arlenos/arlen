...
pub enum Request {
    ...
    /// else's confinement.
    Launch { id: String },
    /// Put the person's resolved theme inside one bottle.
    ///
    /// The ask carries an id and nothing else, deliberately. A `.reg` document
    /// is arbitrary registry content and a bottle is a confinement whose point
    /// is that the broker decides what reaches it, so the daemon generates the
    /// document from the person's own theme rather than importing one a caller
    /// hands it.
    Theme { id: String },
}
...
/// Put a generated theme document into one bottle and import it.
///
/// The `document` is a parameter rather than something this reads, so the
/// refusals are testable without a theme on the build machine - the same reason
/// `launch` takes its spawn. It is generated by the caller from the person's
/// resolved theme; nothing on the socket can supply one.
///
/// `run` is the daemon's one confined runner, the same one a boot goes through:
/// it either finished or it says why not, and this does not try to tell a
/// `regedit` that refused the document from a `bwrap` that would not start,
/// because the runner does not distinguish them and inventing the difference
/// here would be a claim about something nobody measured.
pub fn theme(
    bottles_dir: &Path,
    id: &str,
    document: &str,
    ...
    let bottle = match load_bottle(bottles_dir, id) {
        Ok(b) => b,
        Err(RegistryError::BadId(_)) => return Response::Refused { problem: Problem::BadId },
        Err(RegistryError::NoSuchBottle(_)) => {
            return Response::Refused { problem: Problem::NoSuchBottle }
        }
        Err(_) => return Response::Refused { problem: Problem::Unreadable },
    };
    ...
    let argv = match crate::theme::theme_argv(&bottle, usr, runtime_dir, exists) {
        Ok(v) => v,
        Err(LaunchError::NoRuntime(_)) => return Response::Refused { problem: Problem::NoWine },
        Err(LaunchError::UnmetDrives(_)) => {
            return Response::Refused { problem: Problem::DrivesUnmet }
        }
    ...
    match run(&argv) {
        ...
        // rather than handed a token that sends somebody to look at the prefix.
        Err(why) => {
            tracing::warn!(bottle = id, %why, "the theme did not import");
...
pub fn handle_request(bottles_dir: &Path, request: &Request) -> Option<Response> {
    Some(match request {
        ...
        | Request::Create { .. }
        | Request::Install { .. }
        // And a theme needs it twice over: it runs Wine on this machine, and the
        // document it imports is generated from the person's own theme files,
        // neither of which a dispatch pure over one directory can reach.
        | Request::Theme { .. }
...
mod tests {
    ...
    fn a_theme_says_which_thing_stopped_it_and_never_half_reports() {
        ...
        let prefix = dir.path().join("b1/pfx");
        std::fs::create_dir_all(prefix.join("drive_c")).unwrap();
        std::fs::write(
            dir.path().join("b1/bottle.toml"),
            format!(
                "id = \"b1\"\nprefix_root = \"{}\"\negress = \"none\"\ngrants = []\n",
                prefix.display()
            ),
        ...
        .unwrap();
        let usr = std::path::Path::new("/usr");
        let run_dir = std::path::Path::new("/run/user/1000");
        ...
        // A prefix that was never booted. The document must not be written into
        // a directory that is not a prefix, and nothing may run.
        let unbooted = dir.path().join("b2");
        std::fs::create_dir_all(&unbooted).unwrap();
        std::fs::write(
            unbooted.join("bottle.toml"),
            format!(
                "id = \"b2\"\nprefix_root = \"{}\"\negress = \"none\"\ngrants = []\n",
                unbooted.join("pfx").display()
            ),
        )
        .unwrap();
        assert_eq!(
        ...
        assert_eq!(reply, Response::Themed { imported: true, font_registered: false });
        assert_eq!(
            std::fs::read_to_string(crate::theme::reg_host_path(&prefix)).unwrap(),
            "REGEDIT4\r\nhello\r\n"
        );
        let argv = seen.into_inner();
        assert!(
